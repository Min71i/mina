...
لشراء نسخه مراسلةة المطور 👁‍🗨',
                  'reply_markup'=>json_encode([
                      'inline_keyboard'=>[
                          [['text'=>'▫️| 『 المطور』','url'=>'@F_7_U']],
                          [['text'=>"قناة المطور  ℡ᴖ̈💜✨⋮", 'url'=>"@PR2_NET"]],
                          [['text'=>"قناتي", 'url'=>"@PR2_NET"]]
                      ]
                  ])
              ]);   
				}
          ...
          if($data == 'login'){
            ...
            $keyboard = [];
             foreach ($accounts as $account => $v) {
              ...
              $bot->answerCallbackQuery([
							'callback_query_id'=>$update->callback_query->id,
							'text'=>"انا مشفتكش كده ضيف ياض حساب وهمي",
							'show_alert'=>1
						]);
            }
            ...
            $keyboard = [];
             foreach ($accounts as $account => $v) {
              ...
              $bot->answerCallbackQuery([
							'callback_query_id'=>$update->callback_query->id,
							'text'=>"يبني هتموتني بذكائك ده ضيف حساب وهمي ارجوك",
							'show_alert'=>1
						]);
            }
          }elseif($data == 'stop'){
            ...
            $keyboard = [];
             foreach ($accounts as $account => $v) {
              ...
              $bot->answerCallbackQuery([
							'callback_query_id'=>$update->callback_query->id,
							'text'=>"ابو ام دي شغلانه يبني ضيف وهمي",
							'show_alert'=>1
						]);
            }
          }elseif($data == 'stopgr'){
